<?php
//エラーを画面に表示させる
//ini_set("display_errors", 1);
ini_set('display_errors', "On");
ini_set('error_reporting', E_ALL);

// ファイルの読み込み
require_once('dbconnect.php');
require_once('function.php');

//使うdbを準備し、$stmtに代入する
//DBからデータを取得
$stmt = $dbh->prepare('SELECT * FROM tasks');
//$stmtを実行する
$stmt->execute();

// 実行結果を読み込み、変数に代入
$tasks = $stmt->fetchAll();

?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Todoアプリ</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
</head>

<body>
    <div class="container-fulid">
        <div class="row">
            <div class="col-12">
                <nav class="navbar navbar-dark bg-dark">
                    <a href="index.php" class="navbar-brand">Todo</a>
                    <ul class="nav nav-pills">
                        <li class="nav-item">
                            <a class="nav-link text-light" href="create.php">Create</a>
                        </li>
                        <li class="nav-item">
                            <form class="form-inline">
                                <input class="form-control mr-sm-2" type="search" placeholder="Search" aria-label="Search">
                                <button class="btn btn-outline-light my-2 my-sm-0" type="submit">Search</button>
                            </form>
                        </li>
                    </ul>

                </nav>
            </div>
        </div>

        <div class="row p-3">
            <!-- 先頭データから順に終わるまで繰り返す処理 -->
            <?php foreach ($tasks as $task) : ?>
                <div class="col-sm-6 col-md-4 col-lg-3 py-3 py-3">
                    <div class="card">
                        <img src="https://picsum.photos/200" class="card-img-top" alt="...">
                        <div class="card-body">
                            <!-- h関数にtitleに入力された変数値が代入された変数を出力 -->
                            <h5 class="card-title"><?= h($task["title"]); ?></h5>
                            <p class="card-text">
                                <!-- h関数にcontentsで入力された変数値が代入された値を出力 -->
                                <?= h($task["contents"]); ?>
                            </p>
                            <div class="text-right d-flex justify-content-end">
                                <!-- EDITをクリックしてedit.phpに移動するとき、指定されたidへ移動するように関数を組む -->
                                <a href="edit.php?id=<?= h($task['id']); ?>" class="btn text-success">EDIT</a>
                                <form action="delete.php" method="post">
                                    <!-- DELETEをクリックして削除するとき、削除される場所とデータベースをリンクさせるために関数を組む -->
                                    <input type="hidden" name="id" value="<?= h($task['id']); ?>">
                                    <button type="submit" class="btn text-danger">DELETE</button>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
    </div>


</body>

</html>